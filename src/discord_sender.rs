...
impl DiscordSender {
    pub fn send_file<P: AsRef<Path>>(webhook_url: &str, file_path: P, message: Option<&str>) -> Result<()> {
        ...
        if !response.status().is_success() {
            ...
            let error_text = response.text().unwrap_or_else(|_| "No error message".to_string());
...
mod tests {
    ...
    fn test_send_file_success() -> Result<()> {
        ...
        let _m = server.mock("POST", "/api/webhooks/test")
            .with_status(204)
            .create();
    ...
    fn test_send_file_discord_error() -> Result<()> {
        ...
        assert!(result.unwrap_err().to_string().contains("Discord API returned error"));
    ...
    async fn test_send_file_async_success() -> Result<()> {
        ...
        DiscordSender::send_file_async(&webhook_url, temp_file.path(), Some("Test message")).await?;
